...
export const PreProcessorRunner = () => {
    const run = async (request: PreProcessorRunnerRequest): Promise<PreProcessorRunnerResponse> => {
        ...
        if (missingSteps.length > 0) throw new Error(`cannot find these pre process steps: ${missingSteps.join(',')}`);
        ...
        return await preProcessors.reduce(async (a, c) => {
            ...
            return { ...a, text: response.text, explainSteps: [...explainSteps, { before: text, after: response.text, elapsed, start, name: `${c.name}PreProcessor` } as BeforeAfterExplainStep] };
